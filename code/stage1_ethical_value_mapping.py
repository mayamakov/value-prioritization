...
for i, doctor_key in enumerate(ALL_DOCTORS, 1):
    # Reproducibility: reseed per rater so results do not depend on processing order.
    # NOTE: the SHAP artifact shipped with this repository (analysed in the paper) was
    # generated by the original, order-dependent run and remains the canonical artifact
    # of record; regenerating with --with-shap will produce equivalent but not
    # byte-identical SHAP values. All pairwise-based results are unaffected.
